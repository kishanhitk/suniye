...
final class AppState {
    ...
    var llmKeyStatusText: String {
        hasOpenRouterAPIKey ? "API key: \(llmMaskedAPIKey)" : "API key: missing"
    }
    ...
    func refreshLLMKeyStatus() {
        do {
            if let key = try keychainService.getOpenRouterKey(), !key.isEmpty {
                hasOpenRouterAPIKey = true
                llmMaskedAPIKey = Self.maskedAPIKey(for: key)
            } else {
    ...
    private static func maskedAPIKey(for key: String) -> String {
        guard !key.isEmpty else {
            return ""
        }
        return String(repeating: "*", count: min(max(key.count, 8), 12))
    }
    ...
    func setLaunchAtLoginEnabled(_ enabled: Bool) {
        do {
            ...
            launchAtLoginEnabled = launchAtLoginService.isEnabled()
            launchAtLoginError = error.localizedDescription + " Install Suniye in /Applications and retry."
        }
        onStateChange?()
    }

...
start_marker="E2E_LLM_SUCCESS_START_$(date +%s)"
echo "${start_marker}" >> "${LOG_FILE}"

pkill -f '/Suniye.app/Contents/MacOS/Suniye' || true
sleep 1

"${BIN_PATH}" --e2e-llm-success >/dev/null 2>&1 &
app_pid=$!
...
if [[ "${exited}" != "1" ]]; then
  echo "LLM success smoke did not terminate in time" >&2
...
require_log "llm e2e forced success"
require_log "e2e llm smoke result mode=success changed=true"

echo "E2E LLM success passed."

...
require_file "${ROOT_DIR}/VibeStoke/VibeStoke-Bridging-Header.h"
if command -v rg >/dev/null 2>&1; then
  SWIFT_COUNT="$(rg --files VibeStoke -g '*.swift' | wc -l | tr -d ' ')"
else
  SWIFT_COUNT="$(find VibeStoke -type f -name '*.swift' | wc -l | tr -d ' ')"
fi
if [[ "${SWIFT_COUNT}" -eq 0 ]]; then
  echo "No Swift source files found under VibeStoke/" >&2
  exit 1
fi

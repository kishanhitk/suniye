...
- [Verified] Each request contains the original instruction, one observation, all discovered available apps, accumulated action results, accumulated failure messages, and a one-based iteration. (`Suniye/Services/ComputerUseAgentModels.swift:31-53`, `Suniye/Services/ComputerUseAgent.swift:176-183`)
...
- [Verified] The loop order is observe/activate, ask the model for one decision, optionally authorize and execute one action, wait one second, then observe again. A target decision changes the application ID and immediately starts the next observation. (`Suniye/Services/ComputerUseAgent.swift`, `Suniye/Services/ComputerUseModelClient.swift`)
...
- [Verified] The default policy has no denied or forbidden bundle IDs and no persistent-approval risks, so any app with a non-empty bundle ID is allowed with `.once` scope. (`Suniye/Services/ComputerUsePolicyService.swift:9-21`, `Suniye/Services/ComputerUsePolicyService.swift:37-70`)
...
- [Verified] Before input, the action service validates approval/policy, exact target and generation binding, current Accessibility permission, and target activation. (`Suniye/Services/ComputerUseActionService.swift:37-67`)

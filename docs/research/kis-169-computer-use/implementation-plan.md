...
Status: implementation reset. `kis-169-computer-use-parity` starts from current `origin/main` and
contains no Computer Use production code or tests. The Phase 0 through Phase 5 sections below are
the preserved historical plan and implementation record from `kis-169-computer-use`; they are not
claims about this branch. The recovered client request loop and native-helper algorithms remain
the authoritative reference for the fresh implementation.

The phase sections below preserve the incremental design record. The 2026-08-09 native parity plan
...
- `[Validated]` Phase 2: AX observation tree, revision identity/diffs, and screenshot capture;
  1,012 tests, 2 skipped, 0 failures; 95.14% gated coverage; E2E preflight and smoke pass.
- `[Next]` Native actions; then model/agent integration, permissions, cancellation/intervention,
